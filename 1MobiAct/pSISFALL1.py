...
names1 = ["x_acc", "y_acc", "z_acc", "x_gyr", "y_grc", "z_grc",'Azimuth','Pitch','Roll']
...
df4.to_csv(r'XYZ/YSis1falls.csv', index=0)
...
cols_swap = lx
df4.insert(0, 'labelz', cols_swap, True)
df4.drop('Sisfall_Label_X4', axis=1, inplace=True)  # Remove Colum
df4.drop(df4.loc[df4['labelz'] ==
         'labelz'].index, inplace=True)  # Remove Row
df4 = pd.DataFrame(df4)
df4.to_csv(r'XYZ/YSis1ADLs.csv', index=0)


print(
    '====================================|COMBINED_SISFALL|===========================================[4]=============')
df31 = pd.read_csv('XYZ/YSis1Falls.csv', header=0, delimiter=',')
df32 = pd.read_csv('XYZ/YSis1ADLs.csv', header=0, delimiter=',')
...
ax.axis('off')
ax.axis('tight')
ax1 = ax.table(cellText=df2.values, colLabels=df2.columns, loc='center', colColours=kalas, fontsize=zed)
ax1 = ax.table(cellText=df2.values, colLabels=df2.columns, loc='center',fontsize=zed)
ax1.auto_set_font_size(False)
ax1.set_fontsize(zed)
plt.title('DATASET_1_SISFALL | # Headers= '+str(c) + ' / '+str(len(df33.columns)) +
          ': # Records='+str(l)+'/'+str(len(df33))+'|', fontsize=zed, color='green', fontweight="bold")
plt.tight_layout()
plt.savefig('../UXviews/table2/T8A.png')
plt.show()
df2 = pd.DataFrame(df34)
print(df2.shape)
df2.to_csv(r'XYZ/YSis1ALLS.csv', index=0)
...
ax.axis('off')
ax.axis('tight')
ax1 = ax.table(cellText=df4.values, colLabels=df4.columns, loc='center', colColours=kalas, fontsize=zed)
ax1 = ax.table(cellText=df4.values, colLabels=df4.columns, loc='center', fontsize=zed)
ax1.auto_set_font_size(False)
ax1.set_fontsize(zed)
plt.title('DATASET_1_SISFALL | # Headers= '+str(c) + ' / '+str(len(df2.columns)) +
          ': # Records='+str(l)+'/'+str(len(df2))+'|', fontsize=zed, color='green', fontweight="bold")
plt.tight_layout()
plt.savefig('../UXviews/table2/T8B.png')
plt.show()
print('============================================|SISFALL_THREE_SMALL|==============================|6|=============')
df10 = pd.read_csv('XYZ/YSis1ALLS.csv', header=0, delimiter=',')
print(df10)
print(df10.columns)
df11=df10
df4 = df10.iloc[0:25000:, -15:]  # 22-Records, last-15X headers
df4.loc[df4['labelz'].isin([0])]  # If contains one add to df4
df4.to_csv(r'XYZ/YSisFall21BZ.csv', header=0, index=0)

df6 = df10.iloc[25000:40000:, -15:]  # 22-Records, last-15X headers
df6.loc[df6['labelz'].isin([1])]
df6.to_csv(r'XYZ/YSisFall22BP.csv', header=0, index=0)

df7 = df10.iloc[40000:50000:, -15:]  # 22-Records, last-15X headers
df7.loc[df7['labelz'].isin([2])]
df7.to_csv(r'XYZ/YSisFall23B2.csv', header=0, index=0)
...
df7.to_csv(r'XYZ/YSisFall33SZ.csv', header=0, index=0)


print('=====================================|SISFALL_REMOVE_HEADERS|==================================|8|=============')

df10.to_csv(r'XYZ/YSis1ALLS.csv', header=0, index=0)
print(df7.head(5))
